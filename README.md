...
지표 선택 및 다차원 시각화 기능 
1. 사용자가 '종합 보건 위험 등급', '미세먼지(PM10)', '초미세먼지(PM2.5)' 원하는 지표를 선택할 수 있도록 버튼 UI를 구현하였습니다. 
...
4.우측 패널에는 선택한 자치구(예: 중구)의 AI 모델 분석 근거를 연동하여, RandomForest 기반의 4단계 위험 등급(안전•보통•위험•고위험) 및 주변 병원정보를 실시간으로 출력하도록 구현하였습니다.
