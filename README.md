...
* **정밀 위험도 예측**: 미세먼지의 체내 잔류 특성(1~3일)을 반영하여, 단순 현재 수치가 아닌 누적 시계열 데이터와 지역 산업 특성을 결합한 RandomForest 기반 4단계(안전~고위험) 예측 엔진을 구현합니다.
* **실시간 방역 지표 수치화**: 마스크 착용률을 실시간 객체 탐지하여, 추상적인 보건 수칙을 구체적인 '지역별 마스크 관제 통계' 데이터로 변환합니다.
* **개인화된 맞춤형 보건 컨텍스트 제공**: 지역별 실시간 데이터와 연동된 LLM 챗봇 파이프라인을 구축하여 맞춤형 행동 가이드를 제공합니다.
